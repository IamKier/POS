      ...
      <header className="no-print flex h-14 shrink-0 items-center gap-4 border-b border-line bg-surface px-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {isManager ? (
            <button
              onClick={onOpenAdmin}
              className="flex h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm font-medium text-ink transition-colors active:bg-surface-2"
            >
          ...
          {requiresAuth ? (
            <button
              onClick={() => signOutNow()}
              title="Sign out"
              className="flex size-10 items-center justify-center rounded-xl border border-line text-muted transition-colors active:bg-surface-2"
            >
