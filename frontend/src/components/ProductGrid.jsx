 ...
 * gets the top two thirds of the card. Without one the card shows the
 * initials instead, which still gives the eye something to aim at.
 */
      ...
      {products.map((product) => {
        ...
        return (
          <button
            ...
            className={`group flex flex-col overflow-hidden rounded-sm border bg-surface text-left transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45 ${
              added
          ...
          >
            <div className="relative aspect-[4/3] w-full overflow-hidden bg-surface-2">
              {product.image ? (
                ...
                <span className="flex size-full items-center justify-center text-2xl font-semibold text-line-strong">
                  {initials(product.name)}
                </span>
              )}

              {product.modifierGroups?.length ? (
                <span className="absolute top-2 right-2 rounded-sm bg-white/90 p-1.5">
              ...
              {out ? (
                ...
                <span className="absolute right-2 bottom-2 rounded-sm bg-warn px-2 py-0.5 text-xs font-medium text-white">
                  {product.stock} left
                </span>
              ) : null}
            </div>
