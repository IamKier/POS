    ...
    <aside className="no-print flex w-full max-w-md min-w-0 shrink-0 flex-col border-l border-line bg-surface md:w-[24rem] lg:w-[27rem]">
      <header className="flex h-16 items-center justify-between border-b border-line px-4">
        <h2 className="flex items-center gap-2 text-[15px] font-semibold text-ink">
          ...
          Order
          {totals.itemCount > 0 ? (
            <span className="tnum rounded-sm bg-surface-2 px-2.5 py-1 text-sm text-muted">
              {totals.itemCount}
            </span>
          ) : null}
        </h2>
        {!empty ? (
          <button
            onClick={onClear}
            className="rounded-sm px-3 py-2 text-sm font-medium text-muted transition-colors hover:text-bad active:bg-surface-2"
          >
            Clear
          </button>
        ) : null}
              ...
              <li key={line.id} className="px-4 py-3">
                ...
                <div className="mt-2.5 flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="tnum h-12 min-w-14 rounded-sm bg-surface-2 px-3 text-lg font-semibold text-ink transition-colors active:bg-surface-3"
                  >
                    {line.qty}
                  </button>
      ...
      <div className="border-t border-line px-4 py-3">
        <input
          ...
          className="mb-3 h-11 w-full rounded-sm bg-surface-2 px-3 text-sm text-ink outline-none placeholder:text-muted/70 focus:ring-2 focus:ring-accent-solid/20"
        />
        ...
        <dl className="space-y-2 text-[15px]">
          ...
          <div className="flex items-center justify-between gap-2">
            <button
              ...
              className="-ml-2 flex items-center gap-1.5 rounded-sm px-2 py-1.5 text-muted transition-colors hover:text-accent active:bg-surface-2"
            >
              {discountLocked ? (
          ...
          {settings.statutoryDiscount ? (
            <div className="flex items-center justify-between gap-2">
              <button
                ...
                className={`-ml-2 flex min-w-0 items-center gap-1.5 rounded-sm px-2 py-1.5 transition-colors hover:text-accent active:bg-surface-2 ${
...
function StepButton({ label, className = "", ...props }) {
  return (
    <button
      aria-label={label}
      title={label}
      className={`flex size-12 shrink-0 items-center justify-center rounded-sm border border-line text-ink transition-colors active:bg-surface-2 ${className}`}
      {...props}
    />
  );
}
